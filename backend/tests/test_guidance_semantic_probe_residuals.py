    ...
    *,
    ...
    if action is not GuidanceAction.NONE:
        needles = {
            ...
            GuidanceAction.REAFFIRM: ("maintain", "reaffirm", "continues to expect"),
...
def test_full_year_guidance_value_cannot_be_serialized_as_quarter_guidance():
    ...
    f = _fact(
        ...
        metric_text="Adjusted EPS",
        value_text="$4.22 to $4.42",
...
def test_realized_revenue_edged_down_is_not_lower_guidance():
    text = "Investment Management: Revenue edged down 1% to $155 million. The decline was driven by lower incentive fees."
    d = _doc("CBRE", text)
    f = _fact(
        d,
        metric=GuidanceMetric.REVENUE,
        ...
        kind=GuidancePeriodKind.QUARTER,
        metric_text="Revenue",
        value_text="$155 million",
        period_text="Q4FY2024" if "Q4FY2024" in text else "Investment Management",
        low=155e6,
        high=155e6,
        action=GuidanceAction.LOWER,
    )
    # The historical movement and lower incentive fees are not a management guidance cut.
    assert not GuidanceEvidenceBinder().bind(f, d).accepted
...
def test_fx_headwind_amount_is_not_company_revenue_guidance():
    ...
    f = _fact(
        ...
        period_text="First Quarter of 2025",
        low=5e6,
        high=5e6,
        action=GuidanceAction.INITIATE,
    )
