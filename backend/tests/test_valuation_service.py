...
def test_historical_median_pe_sets_frozen_valuation_inputs_and_conservative_upside():
    ...
    # Historical normalized value is 200, below the 240 analyst target, so
    # expected swing headroom uses the lower anchor: 200 / 160 - 1 = 25%.
    ...
    assert score.score == 14  # 8 upside + 6 support under frozen SOE bands


def test_historical_overvaluation_cannot_be_overridden_by_bullish_analyst_target():
    history, ends = _history(metric=25.0)
    ...
    assert score_valuation(result).score == 0
