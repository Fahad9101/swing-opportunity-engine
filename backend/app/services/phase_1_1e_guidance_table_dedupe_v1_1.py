...
_PREFIX = (
    ...
    "normalized_explicit_guidance_scope;",
    "phase_1_1e_run94_explicit_period_authority=",
)
...
def dedupe_guidance_records_table_normalized(
    ...
    """Global guidance dedupe with structured normalized records as authoritative.

    Generic Round-8 evidence remains fully hardened. When a normalized
    comparative-table, explicit-scope, or Run-94 issuer-authority record exists,
    conflicting generic records from the same ticker/source timestamp/metric/
    fiscal scope are removed before ledger use. This prevents a later global
    dedupe pass from re-running prose binders over evidence whose scope was
