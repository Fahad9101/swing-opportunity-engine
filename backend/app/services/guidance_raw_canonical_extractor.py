...
_VALUE_OWNER_PATTERNS: list[tuple[GuidanceMetric | str, re.Pattern[str]]] = [
    (GuidanceMetric.GROSS_MARGIN, re.compile(r"\b(?:(?:adj(?:usted)?|non[- ]GAAP)\s+)?gross(?:\s+profit)?\s+margin\b", re.I)),
    (GuidanceMetric.OPERATING_MARGIN, re.compile(r"\b(?:(?:adj(?:usted)?|non[- ]GAAP)\s+)?operating\s+margin\b", re.I)),
    (GuidanceMetric.FCF, re.compile(r"\b(?:free\s+cash\s+flow|FCF)\b", re.I)),
    (GuidanceMetric.EPS, re.compile(r"\b(?:(?:adj(?:usted)?|non[- ]GAAP|GAAP)\s+)?(?:diluted\s+)?(?:EPS|earnings\s+per\s+(?:common\s+)?share)\b", re.I)),
    (GuidanceMetric.EBITDA, re.compile(r"\b(?:(?:adj(?:usted)?|non[- ]GAAP)\s+)?EBITDA\b(?!\s+margin)", re.I)),
    (GuidanceMetric.REVENUE, re.compile(r"\b(?:total\s+(?:product\s+)?revenue|consolidated\s+revenue|net\s+sales|revenues?)\b", re.I)),
    ("net_income", re.compile(r"\b(?:(?:adjusted|GAAP)\s+)?net\s+(?:income|loss)\b", re.I)),
    ("operating_cash", re.compile(r"\b(?:net\s+cash\s+provided\s+by\s+operating\s+activities|operating\s+cash\s+flow)\b", re.I)),
...
_MONEY_SCALE_UNITS = {
    "b": GuidanceUnit.USD_BILLION, "bn": GuidanceUnit.USD_BILLION, "billion": GuidanceUnit.USD_BILLION,
    "m": GuidanceUnit.USD_MILLION, "mm": GuidanceUnit.USD_MILLION, "million": GuidanceUnit.USD_MILLION,
    "thousand": GuidanceUnit.USD_THOUSAND,
}
_MONEY_UNIT_SCALE = {
    GuidanceUnit.USD: 1.0, GuidanceUnit.USD_THOUSAND: 1_000.0,
    GuidanceUnit.USD_MILLION: 1_000_000.0, GuidanceUnit.USD_BILLION: 1_000_000_000.0,
}

_FROM_TO_MONEY = re.compile(
    r"\bfrom\s+(?P<d1>\$)?\s*(?P<old>-?\d[\d,]*(?:\.\d+)?)\s*(?P<s1>billion|million|thousand|bn|mm|m|b)?\s+"
    r"(?:to|through)\s+(?P<d2>\$)?\s*(?P<new>-?\d[\d,]*(?:\.\d+)?)\s*(?P<s2>billion|million|thousand|bn|mm|m|b)?\b", re.I,
)
_BY_TO_MONEY = re.compile(
    r"\bby\s+(?P<d1>\$)?\s*(?P<delta>\d[\d,]*(?:\.\d+)?)\s*(?P<s1>billion|million|thousand|bn|mm|m|b)?\s*,?\s*"
    r"(?:to|bringing\s+(?:the\s+)?(?:guidance|outlook)\s+to)\s+(?P<d2>\$)?\s*(?P<target>-?\d[\d,]*(?:\.\d+)?)\s*"
    r"(?P<s2>billion|million|thousand|bn|mm|m|b)?\b", re.I,
)
_FROM_TO_PERCENT = re.compile(r"\bfrom\s+(?P<old>\d{1,3}(?:\.\d+)?)\s*%\s+(?:to|through)\s+(?P<new>\d{1,3}(?:\.\d+)?)\s*%", re.I)
_BY_TO_PERCENT = re.compile(
    r"\bby\s+(?P<delta>\d{1,3}(?:\.\d+)?)\s*(?:percentage\s+points?|%)\s*,?\s*"
    r"(?:to|bringing\s+(?:the\s+)?(?:guidance|outlook)\s+to)\s+(?P<target>\d{1,3}(?:\.\d+)?)\s*%", re.I,
)
_PREVIOUS_NOW_MONEY = re.compile(
    r"\bpreviously\b[^$%]{0,100}(?P<d1>\$)?\s*(?P<old>-?\d[\d,]*(?:\.\d+)?)\s*(?P<s1>billion|million|thousand|bn|mm|m|b)?"
    r"[^$%]{0,100}\b(?:now|currently|today)\b[^$%]{0,100}(?P<d2>\$)?\s*(?P<new>-?\d[\d,]*(?:\.\d+)?)\s*"
    r"(?P<s2>billion|million|thousand|bn|mm|m|b)?\b", re.I,
)
_PREVIOUS_NOW_PERCENT = re.compile(
    r"\bpreviously\b[^%]{0,100}(?P<old>\d{1,3}(?:\.\d+)?)\s*%[^%]{0,100}\b(?:now|currently|today)\b[^%]{0,100}"
    r"(?P<new>\d{1,3}(?:\.\d+)?)\s*%", re.I,
)
...
def _owned_directional_action(clause: str, anchor: int, metric_text: str, metric) -> GuidanceAction:
    ...
    candidates = [item for item in mentions if item.metric is metric and item.text.lower() == metric_text.lower()]
    if not candidates:
    ...
    for action, pattern in _ACTION_PATTERNS:
        for match in pattern.finditer(clause):
            if match.end() <= current.start:
                ...
                if gap > 120 or any(match.end() <= item.start < current.start for item in mentions):
                    continue
                owned.append((gap, action))
            elif match.start() >= current.end:
                gap = match.start() - current.end
                if gap > 120 or any(current.end < item.start <= match.start() for item in mentions):
                    continue
                owned.append((gap, action))
            else:
...
def _canonical_period_binding(clause: str, anchor: int, mention) -> _PeriodBinding | None:
    ...
    def add(binding: _PeriodBinding) -> None:
        ...
        if binding.end <= anchor:
            explicit.append((0, anchor - binding.end, binding))
        elif binding.start >= mention_end:
            explicit.append((1, binding.start - mention_end, binding))
        else:
            explicit.append((0, 0, binding))
    ...
    for pattern in _CANONICAL_QUARTER:
        for match in pattern.finditer(clause):
            token = match.group(1); q = token if token.isdigit() else _QUARTER_WORD[token.lower()]
    ...
    if explicit:
        ...
        if pool:
            ...
            tied = [item for item in pool if item[0] == pool[0][0] and item[1] == pool[0][1]]
            periods = {item[2].period for item in tied}
            return tied[0][2] if len(periods) == 1 else None
    left = max(0, anchor - 150); right = min(len(clause), mention_end + 120)
    bare: list[tuple[int, int, _PeriodBinding]] = []
    for match in _BARE_YEAR.finditer(clause[left:right]):
        start, end = left + match.start(), left + match.end()
        if re.search(r"(?:Q[1-4]|[1-4]Q)\s*(?:FY)?\s*'?\s*$", clause[max(0, start - 14):start], re.I):
            continue
        if not _FORWARD_SIGNAL.search(clause[max(0, start - 90): min(len(clause), end + 110)]):
            continue
        binding = _PeriodBinding(f"FY{int(match.group(1))}", GuidancePeriodKind.FULL_YEAR, match.group(0), start, end)
        if not _admissible_period_candidate(clause, anchor, binding):
            continue
        bare.append((0, anchor - end, binding) if end <= anchor else (1, max(0, start - mention_end), binding))
    if not bare:
        return None
    preceding = [item for item in bare if item[0] == 0]; pool = preceding or [item for item in bare if item[1] <= 100]
    if not pool:
        return None
    pool.sort(key=lambda item: (item[0], item[1], -item[2].end)); best = pool[0]
    tied = [item for item in pool if item[:2] == best[:2]]; periods = {item[2].period for item in tied}
...
def _unique_explicit_period_from_segment(segment: str) -> _PeriodBinding | None:
    ...
    for pattern in _CANONICAL_FULL_YEAR:
        for match in pattern.finditer(segment):
            year = _normalize_year(match.group(1)); candidates.append(_PeriodBinding(f"FY{year}", GuidancePeriodKind.FULL_YEAR, match.group(0), match.start(), match.end()))
    for pattern in _CANONICAL_QUARTER:
        for match in pattern.finditer(segment):
            token = match.group(1); q = token if token.isdigit() else _QUARTER_WORD[token.lower()]
            year = _normalize_year(match.group(2)); candidates.append(_PeriodBinding(f"Q{q}FY{year}", GuidancePeriodKind.QUARTER, match.group(0), match.start(), match.end()))
    periods = {candidate.period for candidate in candidates}
    return min(candidates, key=lambda candidate: candidate.start) if len(periods) == 1 else None
...
def _value_has_local_metric_owner(clause: str, anchor: int, mention, value) -> bool:
    current_start = anchor; current_end = anchor + len(mention.text)
    ...
    for owner, pattern in _VALUE_OWNER_PATTERNS:
        for match in pattern.finditer(clause):
            if owner is mention.metric and not (match.end() <= current_start or match.start() >= current_end): continue
            gap = _span_gap(match.start(), match.end(), value.start, value.end)
            if nearest_other is None or gap < nearest_other[0]: nearest_other = (gap, owner)
    if nearest_other is None: return True
    other_gap, other_owner = nearest_other
    return True if other_owner is mention.metric else current_gap < other_gap
...
def _value_is_historical_actual(clause: str, anchor: int, value) -> bool:
    ...
    if not actuals: return False
    latest_actual = actuals[-1]; after_actual = before[latest_actual.end():]
    if _FORWARD_SIGNAL.search(after_actual): return False
    return len(before) - latest_actual.end() <= 90


def _ambiguous_parallel_period_table(clause: str) -> bool:
    return bool(re.search(r"\bthree\s+months\s+ending\b", clause, re.I) and re.search(r"\byear\s+ending\b", clause, re.I) and re.search(r"\b(?:table|reconciliation|guidance)\b", clause, re.I))


def _ambiguous_current_prior_table(clause: str) -> bool:
    return bool(re.search(r"\b(?:updated|current)\b.{0,40}\bguidance\b", clause, re.I) and re.search(r"\bprior\b.{0,40}\bguidance\b", clause, re.I))


def _fact_role(clause: str, value) -> GuidanceFactRole:
    if value is None: return GuidanceFactRole.CURRENT
    before = clause[max(0, value.start - 180):value.start]
    if (re.search(r"\b(?:from\s+(?:our\s+)?)?prior(?:\s+[A-Za-z0-9*.-]+){0,5}\s+(?:guidance|outlook|forecast)\b", before, re.I)
...
def _canonical_scope(clause: str, anchor: int, mention, value) -> tuple[GuidanceScopeKind, str | None]:
    ...
    contribution = _NON_COMPANY_CONTRIBUTION.search(clause[max(0, anchor - 100): min(len(clause), value_end)])
    if contribution:
    ...
    if mention.metric is not GuidanceMetric.REVENUE: return GuidanceScopeKind.COMPANY, None
    label = mention.text.strip(); lower = label.lower()
    if "total product revenue" in lower or "total revenue" in lower or "consolidated revenue" in lower or "net sales" in lower: return GuidanceScopeKind.COMPANY, label
    if "segment revenue" in lower: return GuidanceScopeKind.SEGMENT, label
    if "product revenue" in lower and "total product revenue" not in lower: return GuidanceScopeKind.PRODUCT, label
    prefix = re.split(r"[.;:•|\n\r]", clause[max(0, anchor - 70):anchor])[-1].strip()
    if _PRODUCT_REVENUE_QUALIFIER.search(prefix): return GuidanceScopeKind.PRODUCT, prefix
    segment_match = _SEGMENT_REVENUE_QUALIFIER.search(prefix)
    if segment_match: return GuidanceScopeKind.SEGMENT, segment_match.group(0).strip()
    return GuidanceScopeKind.COMPANY, None


def _unit_from_scale(scale: str | None, metric: GuidanceMetric, *, dollar: bool) -> GuidanceUnit:
    if metric is GuidanceMetric.EPS: return GuidanceUnit.USD_PER_SHARE
    if scale: return _MONEY_SCALE_UNITS.get(scale.lower(), GuidanceUnit.UNKNOWN)
...
def _previous_now_value_pair(clause: str, mention) -> tuple[_ValueBinding | None, _ValueBinding | None]:
    if mention.metric in {GuidanceMetric.GROSS_MARGIN, GuidanceMetric.OPERATING_MARGIN}:
        ...
        return (_ValueBinding(new, new, GuidanceUnit.PERCENT, GuidanceValueKind.ABSOLUTE_LEVEL, match.group("new") + "%", match.start("new"), match.end("new") + 1), _ValueBinding(old, old, GuidanceUnit.PERCENT, GuidanceValueKind.ABSOLUTE_LEVEL, match.group("old") + "%", match.start("old"), match.end("old") + 1))
    if mention.metric not in {GuidanceMetric.REVENUE, GuidanceMetric.EBITDA, GuidanceMetric.FCF, GuidanceMetric.EPS}: return None, None
    match = _PREVIOUS_NOW_MONEY.search(clause)
    if not match: return None, None
    common_scale = match.group("s2") or match.group("s1"); dollar = bool(match.group("d1") or match.group("d2"))
    old_unit = _unit_from_scale(match.group("s1") or common_scale, mention.metric, dollar=dollar); new_unit = _unit_from_scale(match.group("s2") or common_scale, mention.metric, dollar=dollar)
    if old_unit is GuidanceUnit.UNKNOWN or new_unit is GuidanceUnit.UNKNOWN: return None, None
    old = float(match.group("old").replace(",", "")); new = float(match.group("new").replace(",", ""))
    return (_ValueBinding(new, new, new_unit, GuidanceValueKind.ABSOLUTE_LEVEL, match.group("new"), match.start("new"), match.end("new")), _ValueBinding(old, old, old_unit, GuidanceValueKind.ABSOLUTE_LEVEL, match.group("old"), match.start("old"), match.end("old")))
...
def _directional_value_pair(clause: str, anchor: int, mention, action: GuidanceAction) -> tuple[_ValueBinding | None, _ValueBinding | None]:
    if action not in {GuidanceAction.RAISE, GuidanceAction.LOWER}: return None, None
    if mention.metric in {GuidanceMetric.GROSS_MARGIN, GuidanceMetric.OPERATING_MARGIN}:
        by_match = _BY_TO_PERCENT.search(clause)
        if by_match:
            target = float(by_match.group("target")); return _ValueBinding(target, target, GuidanceUnit.PERCENT, GuidanceValueKind.ABSOLUTE_LEVEL, by_match.group("target") + "%", by_match.start("target"), by_match.end("target") + 1), None
        ft = _FROM_TO_PERCENT.search(clause)
        if ft:
            old, new = float(ft.group("old")), float(ft.group("new")); return _ValueBinding(new, new, GuidanceUnit.PERCENT, GuidanceValueKind.ABSOLUTE_LEVEL, ft.group("new") + "%", ft.start("new"), ft.end("new") + 1), _ValueBinding(old, old, GuidanceUnit.PERCENT, GuidanceValueKind.ABSOLUTE_LEVEL, ft.group("old") + "%", ft.start("old"), ft.end("old") + 1)
        return None, None
    if mention.metric not in {GuidanceMetric.REVENUE, GuidanceMetric.EBITDA, GuidanceMetric.FCF, GuidanceMetric.EPS}: return None, None
    by_match = _BY_TO_MONEY.search(clause)
    if by_match:
        scale = by_match.group("s2") or by_match.group("s1"); dollar = bool(by_match.group("d2") or by_match.group("d1")); unit = _unit_from_scale(scale, mention.metric, dollar=dollar)
        if unit is not GuidanceUnit.UNKNOWN:
            target = float(by_match.group("target").replace(",", "")); return _ValueBinding(target, target, unit, GuidanceValueKind.ABSOLUTE_LEVEL, by_match.group("target"), by_match.start("target"), by_match.end("target")), None
    ft = _FROM_TO_MONEY.search(clause)
    if ft and not re.search(r"\brange\s*$", clause[max(0, ft.start() - 24):ft.start()], re.I):
        common_scale = ft.group("s2") or ft.group("s1"); dollar = bool(ft.group("d1") or ft.group("d2"))
        old_unit = _unit_from_scale(ft.group("s1") or common_scale, mention.metric, dollar=dollar); new_unit = _unit_from_scale(ft.group("s2") or common_scale, mention.metric, dollar=dollar)
        if old_unit is not GuidanceUnit.UNKNOWN and new_unit is not GuidanceUnit.UNKNOWN:
            old = float(ft.group("old").replace(",", "")); new = float(ft.group("new").replace(",", ""))
...
def _money_base(value: float | None, unit: GuidanceUnit) -> float | None:
    ...
    scale = _MONEY_UNIT_SCALE.get(unit); return value * scale if scale is not None else None


def _same_economic_identity(left: TypedGuidanceFact, right: TypedGuidanceFact) -> bool:
    left_label = "" if left.scope_kind is GuidanceScopeKind.COMPANY else (left.scope_label or ""); right_label = "" if right.scope_kind is GuidanceScopeKind.COMPANY else (right.scope_label or "")
    return (left.ticker, left.metric, left.fiscal_period, left.accounting_basis, left.scope_kind, left_label, left.role, left.value_kind) == (right.ticker, right.metric, right.fiscal_period, right.accounting_basis, right.scope_kind, right_label, right.role, right.value_kind)


def _suppress_document_fragments(facts: Iterable[TypedGuidanceFact], rejected: list[dict]) -> list[TypedGuidanceFact]:
    items = list(facts); remove: set[int] = set()
    for i, fact in enumerate(items):
        if i in remove or fact.low is None or fact.high is None or fact.low == fact.high: continue
        for j, other in enumerate(items):
            if i == j or j in remove or not _same_economic_identity(fact, other): continue
            if other.low is None or other.high is None or other.low != other.high: continue
            if fact.unit in _MONEY_UNIT_SCALE and other.unit in _MONEY_UNIT_SCALE:
                lo, hi, point = _money_base(fact.low, fact.unit), _money_base(fact.high, fact.unit), _money_base(other.low, other.unit)
            elif fact.unit == other.unit: lo, hi, point = fact.low, fact.high, other.low
            else: continue
            if lo is None or hi is None or point is None: continue
            tolerance = max(1e-8, abs(point) * 1e-8)
            if abs(point - lo) <= tolerance or abs(point - hi) <= tolerance:
                remove.add(j); rejected.append({"reason": "same_document_range_endpoint_fragment", "metric": other.metric.value, "period": other.fiscal_period, "value": point})
    best_by_key: dict[tuple, int] = {}
    for i, fact in enumerate(items):
        if i in remove: continue
        low = _money_base(fact.low, fact.unit) if fact.unit in _MONEY_UNIT_SCALE else fact.low; high = _money_base(fact.high, fact.unit) if fact.unit in _MONEY_UNIT_SCALE else fact.high
        ...
        if prior is None: best_by_key[key] = i; continue
        prior_fact = items[prior]; score = (fact.low != fact.high, fact.unit in {GuidanceUnit.USD_MILLION, GuidanceUnit.USD_BILLION, GuidanceUnit.USD_THOUSAND}); prior_score = (prior_fact.low != prior_fact.high, prior_fact.unit in {GuidanceUnit.USD_MILLION, GuidanceUnit.USD_BILLION, GuidanceUnit.USD_THOUSAND})
        if score > prior_score: remove.add(prior); best_by_key[key] = i
        else: remove.add(i)
...
def extract_canonical_typed_guidance_facts(document: SourceDocument) -> RawTypedGuidanceExtraction:
    text = html_to_text(document.content or ""); facts: list[TypedGuidanceFact] = []; rejected: list[dict] = []; seen: set[tuple] = set()
    for segment in _segments(text):
        mentions = _metric_mentions(segment)
        if not mentions: continue
        for index, mention in enumerate(mentions):
            clause, anchor = _metric_clause(segment, mentions, index); local = clause[max(0, anchor - 140): min(len(clause), anchor + len(mention.text) + 160)]; explicit_forward = _explicit_forward_context(local); local_action = _metric_action(segment, clause, anchor, mention)
            if not explicit_forward and local_action is GuidanceAction.NONE: continue
            if _ambiguous_parallel_period_table(clause): rejected.append({"reason": "ambiguous_parallel_period_table", "metric": mention.metric.value, "evidence": clause[:500]}); continue
            if _ambiguous_current_prior_table(clause): rejected.append({"reason": "ambiguous_current_prior_table", "metric": mention.metric.value, "evidence": clause[:500]}); continue
            previous_now_value, previous_now_prior = _previous_now_value_pair(clause, mention); directional_value, directional_prior = _directional_value_pair(clause, anchor, mention, local_action); value = previous_now_value or directional_value or _bind_value(clause, mention, anchor); explicit_prior = previous_now_prior or directional_prior
            if value is not None and value.low > value.high: rejected.append({"reason": "invalid_reversed_range", "metric": mention.metric.value, "value_text": value.text, "evidence": clause[:500]}); continue
            if value is not None and not _value_has_local_metric_owner(clause, anchor, mention, value): rejected.append({"reason": "metric_value_locality", "metric": mention.metric.value, "value_text": value.text, "evidence": clause[:500]}); continue
            if value is not None and _value_is_historical_actual(clause, anchor, value): rejected.append({"reason": "historical_actual", "metric": mention.metric.value, "value_text": value.text, "evidence": clause[:500]}); continue
            role = _fact_role(clause, value); period = _canonical_period_binding(clause, anchor, mention)
            if period is None and role is GuidanceFactRole.QUOTED_PRIOR: period = _unique_explicit_period_from_segment(segment)
            if period is None: rejected.append({"reason": "ambiguous_or_missing_period", "metric": mention.metric.value, "evidence": clause[:500]}); continue
            if value is None and local_action not in {GuidanceAction.RAISE, GuidanceAction.LOWER, GuidanceAction.REAFFIRM, GuidanceAction.WITHDRAW}: rejected.append({"reason": "missing_bound_value", "metric": mention.metric.value, "evidence": clause[:500]}); continue
            scope_kind, scope_label = _canonical_scope(clause, anchor, mention, value); basis = _basis(segment, mention)
            ...
            if key not in seen: seen.add(key); facts.append(fact)
            if explicit_prior is not None:
                ...
                if prior_key not in seen: seen.add(prior_key); facts.append(prior_fact)
