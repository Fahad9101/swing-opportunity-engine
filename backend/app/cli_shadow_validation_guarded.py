...
from app.services.phase_1_1e_catalyst_evidence_round5_v1_1 import extract_sec_catalyst_candidates_round5
from app.services.phase_1_1e_evidence_hygiene_round3_patch_v1_1 import extract_hard_distress_flags_round3
...
def _guard_numeric_range(text: str, metric: GuidanceMetric, *, anchor: int) -> tuple[float, float, str] | None:
    result = _original_numeric_range(text, metric, anchor=anchor)
    if result is None:
        return None
    low, high, unit = result
    return None if low > high else (low, high, unit)
...
def _safe_index_submissions_payload(
    ...
    *,
    ...
    return _original_index_submissions_payload(ticker, cik, sanitized, allowed_forms=allowed_forms, limit=limit)
